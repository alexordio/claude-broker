...
server.tool(
  "register_session",
  "Register this Claude Code session as a repo. Call this first so the broker knows who you are.",
  {
    repo: z
      .string()
...
server.tool(
  "list_sessions",
  "List all registered repo sessions and when they were last active.",
  {},
  async () => {
...
server.tool(
  "publish_task",
  "Send a task to another repo session. Use this when you need another repo to do something (update types, add endpoint, sync schema, etc).",
  {
    ...
    type: z
      ...
      .describe(
        "Task type: contract (schema/API change), request (action needed), broadcast (info for all), notify (FYI)"
      ),
    title: z.string().describe("Short description of what needs to happen"),
    payload: z
      .string()
      .describe(
        "JSON string with details: schemas, endpoints, file paths, instructions, etc."
      ),
  },
...
server.tool(
  "broadcast_task",
  "Send a task to ALL other registered repos. Use for breaking changes, shared type updates, or announcements that affect everyone.",
  {
...
server.tool(
  "get_pending",
  "Check for pending tasks addressed to your repo. Call this to see if other repos need you to do something.",
  {
...
server.tool(
  "claim_task",
  "Mark a pending task as in-progress. Call this before you start working on a task.",
  {
...
server.tool(
  "submit_result",
  "Submit the result after completing a task. Include what you changed, files modified, any notes for the requesting repo.",
  {
    task_id: z.string().describe("The task ID you completed"),
    result: z
      .string()
      .describe(
        "JSON string with results: files changed, summary of work, any follow-up needed"
      ),
  },
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { ok: true, message: "Task completed", task },
            null,
            2
          ),
        },
...
server.tool(
  "fail_task",
  "Mark a task as failed if you can't complete it. Include the reason so the requesting repo can adjust.",
  {
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { ok: true, message: "Task marked as failed", task },
            null,
            2
          ),
        },
...
server.tool(
  "get_result",
  "Check the status and result of a task you published. Use the task_id you got from publish_task.",
  {
...
server.tool(
  "get_history",
  "Get recent task history for a repo (both sent and received).",
  {
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { ok: true, count: tasks.length, tasks },
            null,
            2
          ),
        },
